                ...
                <div class="page-wrapper">
                    ...
                    <div class="card">
                        <div class="card-header text-white bg-primary">
                            <h1 class="h6 mb-0 text-black-800">Map Toko Suvenir</h1>
                        </div>
                        <div class="card-body">
...
<script>
    navigator.geolocation.getCurrentPosition(function(location) {
        ...
        var data = [
            <?php foreach ($suvenir as $key => $value) { ?> {
                    "lokasi": [<?= $value['latitude']; ?>, <?= $value['longitude']; ?>],
                    "id": "<?= base_url() ?>home/datasuvenir/<?= $value['id_tksuvenir']; ?>",
                    "lat": "<?= $value['latitude']; ?>",
                    "long": "<?= $value['longitude']; ?>",
                    "image": "<img src='<?= base_url('assets/img/suvenir/' . $value['s_image']) ?>' width='100%'>",
                    "nm_tksuvenir": "<?= $value['nm_tksuvenir']; ?>",
                },
            <?php } ?>
        ...
        for (i in data) {
            var nm_tksuvenir = data[i].nm_tksuvenir,
                id = data[i].id,
                lat = data[i].lat,
                long = data[i].long,
            ...
            marker.bindPopup(image +
                '</br>Nama Toko Suvenir : ' + nm_tksuvenir +
