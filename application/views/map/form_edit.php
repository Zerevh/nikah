                        ...
                        <div class="class col-sm-10">
                            <div class="class row">
                                <div class="class col-sm-2">
                                    <img src="<?= base_url('assets/img/wisata/') . $map['image']; ?>" class="img-thumbnail">
                                </div>
                            </div>
                            <div class="form-group row">
                                <div class="col-sm-6">
        ...
        <script>
            function addMapPicker() {
                var mapCenter = ([<?= $map['latitude']; ?>, <?= $map['longitude']; ?>]);
                var map = L.map('map').setView([<?= $map['latitude']; ?>, <?= $map['longitude']; ?>], 17);
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                $('#prov').change(function() {
                    var id = $(this).val();
                    var id_kab = "<?= $map['id_kab']; ?>";
                    $.ajax({
                        url: "<?= base_url('map/get_ubah_kabupaten'); ?>",
                        method: "POST",
                        data: {
                            id: id
                        },
                        ...
                        success: function(data) {
                            ...
                            $('select[name="kab"]').append('<option value="">--Pilih--</option>')
                            $.each(data, function(key, value) {
                                if (id_kab == value.id_kab) {
                                    $('select[name="kab"]').append('<option value="' + value.id_kab + '" selected>' + value.nama_kab + '</option>').trigger('change');
                                } else {
                                    $('select[name="kab"]').append('<option value="' + value.id_kab + '">' + value.nama_kab + '</option>');
                                }
                            });
                ...
                $('.kab').change(function() {
                    var id = $(this).val();
                    var id_kec = "<?= $map['id_kec']; ?>";
                    $.ajax({
                        url: "<?= base_url('map/get_ubah_kecamatan'); ?>",
                        method: "POST",
                        data: {
                            id: id
                        },
                ...
                function get_data_edit() {
                    var id = <?= $map['id']; ?>;
                    $.ajax({
                        url: "<?= base_url('map/get_data_edit'); ?>",
                        method: "POST",
                        data: {
                            id: id
                        },
                        ...
                        success: function(data) {
                            $.each(data, function(i, item) {
                                ...
                                $('[name="prov"]').val(data[i].id_prov).trigger('change');
